  ...
  <q-card class="shadow-11" :style="{ height: height }">
    <q-card-section>
      <div class="text-h6 text-grey-8 text-weight-bolder">
        {{ selected_customer }}
        <q-select outlined v-model="selected_customer" @update:model-value="selectTriger" class="bg-white float-right q-mb-sm " style="width:300px;" :options="product_options" label="Select Product"  />
  ...
  methods: {
    getList() {
      ...
      if (_this.$q.localStorage.has('auth')) {
        ...
        getauth(_this.pathname + 'cust/', {})
          .then(res => {
            _this.barChartOption.dataset = []
            _this.barChartOption.series = []
            _this.product_options = res;
            _this.selected_customer = res[0];
            console.log(_this.selected_customer)
            getauth(_this.pathname + 'sear/' + '?code=' + _this.selected_customer, {})
              .then(res => {
    ...
    selectTriger() {
      ...
      _this.barChartOption.series = []
      console.log(_this.selected_customer)
      getauth(_this.pathname + 'sales/' + '?code=' + _this.selected_customer, {})
        .then(res => {
          _this.barChartOption.dataset = res.dataset;
          _this.barChartOption.series = res.series;
        })
